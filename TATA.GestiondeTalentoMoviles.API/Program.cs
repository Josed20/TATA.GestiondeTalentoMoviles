...
try
{
    ...
    // Mostrar las URLs donde está escuchando la aplicación
    var urls = app.Urls;
...
catch (SocketException ex)
{
    ...
    Console.WriteLine("❌ ERROR DE RED: No se pudo enlazar a la dirección IP especificada.");
    Console.WriteLine($"   Mensaje: {ex.Message}");
    Console.WriteLine($"   Código de error: {ex.ErrorCode}");
    Console.WriteLine("\n💡 Soluciones posibles:");
    Console.WriteLine("   1. Verifica tu configuración de red");
    Console.WriteLine("   2. Usa 'localhost' o '0.0.0.0' en lugar de una IP específica");
    Console.WriteLine("   3. Revisa el archivo launchSettings.json");
    Console.ResetColor();
    Environment.Exit(1);
}
catch (ReflectionTypeLoadException ex)
{
    ...
    Console.WriteLine("Los siguientes ensamblados no pudieron cargarse:");
...
{
    Console.ForegroundColor = ConsoleColor.Red;
    Console.WriteLine($"❌ ERROR FATAL: {ex.GetType().Name}");
    Console.WriteLine($"   Mensaje: {ex.Message}");
